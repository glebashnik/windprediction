...
This script generates datasets for the models by extracting the desired 
features from the rawdata files and generates the necessary target.
The datasets can be manipulated by commenting out/in features
...
def generate_bessaker_dataset_single_target(tek_path, arome_path):
    ...
    return pd.concat([
        ...
        # Sum production and single productions
        single_target,

    ], axis=1).iloc[:-2, :]

# Assemples the txt files given and creates a dataset out of these
