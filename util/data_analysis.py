...
def feature_importance(dataset, scope=None, num_features=20, print_=True, n_jobs=4):

    dataset = dataset.dropna()

    if scope == None or scope >= dataset.shape[0]:
        scope = dataset.shape[0]

    x = dataset.iloc[:scope, :-1].values
    y = dataset.iloc[:scope, -1:].values.ravel()

    features = dataset.columns.values[:-1]

    print('\nFitting random forest with {} samples\n'.format(scope))

    regressor = RandomForestRegressor(bootstrap=True, criterion='mae', max_depth=20,
    ...
    drop_features = dataset.columns[indices[num_features:]].values
...
def prediction(data, testsize=0.15):
    ...
    x_train, x_test, y_train, y_test = train_test_split(
        data[:, :-1], data[:, -1:], test_size=testsize, random_state=666)
...
if __name__ == '__main__':
    ...
    datapath_advanced = os.path.join(
        '..', 'data', 'Bessaker Vindpark', 'data_bessaker_advanced.csv')

    dataset = read_csv(datapath_advanced, index_col=0, sep=';')

    print(dataset.columns)

    new_data = dataset[['BESS-Bessakerfj.-G2-T4015A3 -0104',
                        'BESS-Bessakerfj.-G2-T4015A3 -0104']].head()
    ...
    print('Predicting with pearson correlations above {}, gives {} features'.format(
        treshold, pearson_dataset.shape[1]))
    pearson_pred, y_test = prediction(pearson_dataset.values, testsize)

    # Compare the prediction methods
    visualize([y_test, raw_pred, PCA_pred, pearson_pred],
              legend=['GT', 'Raw', 'PCA', 'Pearson'], scope=0.2)
